...
function Table() {
    ...
    const thPreset = preset != null ? {color : preset.th.textColor, backgroundColor : preset.th.backgroundColor}: {}
    const oddRowPreset = preset != null ? {color : preset.oddRow.textColor, backgroundColor : preset.oddRow.backgroundColor, borderBottom:'1px solid '+ preset.oddRow.bottomSeparatorColor}: {}
    const evenRowPreset = preset != null ? {color : preset.evenRow.textColor, backgroundColor : preset.evenRow.backgroundColor, borderBottom:'1px solid '+ preset.evenRow.bottomSeparatorColor}: {}
    ...
    return (
        ...
        <tbody>
          {[...rowsToDisplay].map((datarow, index) => (
            <tr style={isRowOdd(index) ? oddRowPreset : evenRowPreset} key={'trtable-'+index} className={isRowOdd(index) + isLastRow(index, rowsToDisplay.length-1) /* !!! use css 2*n+1 */}>
