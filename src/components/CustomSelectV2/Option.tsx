...
function Option({index, option} : IProps){
    ...
    return (
        ...
        onMouseDown={() => {activeOption.set(options[index]); listbox.setAsExpanded(false); console.log(document.activeElement?.getAttribute('id'))}} value={option.value}>{option.label}</li>
    )
}
